...
SELECT * FROM Reservation r INNER JOIN theme t ON r.tid = t.tid INNER JOIN User u ON r.uid = u.uid WHERE r.cid = 1 and r.rDate = '2022-11-03' and r.tid = 1 ORDER BY r.tid ASC, case when r.rTime like '%AM%' then 1 when r.rTime like '%PM%' then 2 else 3 end, case when r.rTime like '%12%' then 1 else 2 end ,r.rTime ASC;
